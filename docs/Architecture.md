...
29. **`DENTRO DE ALCANCE, confirmado por Stefano 18/09/2026`** — control de cheques (pantalla nativa de SAP "Saldo de cheque"). Karen B. lo pidió como agregado a Cliente 360, dentro de "comportamiento de pago": **deuda de cliente en cheques, importe total de cheques, cantidad de cheques, promedio de plazo en cheques.** Distinto del punto 1 de esta guía (cheques diferidos entregados y todavía no cobrados, `vencimiento > hoy` — confirmado por Líber) — **no** se pide todavía automatizar la detección de cheques rechazados (eso sigue como iniciativa separada, sin fecha, mencionada en la minuta de Claudia; no confundir alcance). **`TO VERIFY` bloqueante antes de construir:** qué tabla/query real respalda la pantalla nativa "Saldo de cheque" (columnas vistas en captura real: Fecha, Fecha de vencimiento, ID interno, Verificar, Pago recibido, Importe del cheque, País/Región, Banco, Sucursal bancaria, Número de cuenta, Depositado, Endosado, Cancelados, Sucursal) y si es alcanzable por Service Layer o necesita HANA directo (mismo patrón de pregunta que ya se resolvió para `PedidosParaAutorizar`) — no inventar un nombre de tabla SAP sin confirmarlo, mismo criterio que rigió toda esta sección hasta ahora.
30. **`DENTRO DE ALCANCE, confirmado por Líber 18/09/2026`** — agregar una consulta de "estado de cuenta" de un cliente a Cliente 360, más allá de la lista de facturas abiertas que ya existe. **`RESUELTO 18/09/2026` — la SQL real fue aportada por Líber y ya está en este repo, `sql/estado_cuenta_cliente.sql` (copiada literal, sin modificar).** Hallazgos concretos de leer esta SQL con cuidado, antes de implementar nada:
    ...
    - **No devuelve un "Importe" (monto original del documento) separado del "Pendiente".** El reporte impreso muestra ambas columnas (Importe y Pendiente, que coinciden cuando no hubo pago parcial) — esta SQL solo devuelve una columna de saldo pendiente. **`PREGUNTA PARA LÍBER`:** ¿hace falta mostrar el "Importe" original además del "Pendiente", o alcanza con Pendiente + Saldo corrido? Si hace falta, requiere un join adicional (`OINV.DocTotal`/`ORIN.DocTotal`) que esta SQL no trae — no se inventa sin confirmar que vale la pena el join extra.
    - **`PREGUNTA PARA LÍBER`, relacionada con el principio de nunca sumar entre monedas (Sección 5):** si un cliente tiene actividad en más de una moneda (C1 y C2, por ejemplo), ¿el "saldo corrido" debe calcularse por separado para cada moneda (nunca mezclando pesos y dólares en una sola acumulación), o el reporte real de hecho nunca mezcla monedas en la práctica porque cada cliente opera casi siempre en una sola? Necesito un ejemplo real de un cliente con `U_NumeroSN` compartido entre C1 y C2 para confirmar cómo se ve el reporte real en ese caso, antes de decidir el cálculo.
