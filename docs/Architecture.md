...
- **Azure SQL DB** — `PROPOSED`, mismo criterio que los dos hermanos (elegido sobre Table Storage por necesidad real de listar/filtrar/editar): estado de la Bandeja de Autorización, historial de gestiones/comunicaciones del Hub Omnicanal, y cualquier dato propio de la plataforma que no viva en SAP. `TO VERIFY` si se crea dentro del server de propósito general `sql-pontyn-prod` (ya existente, usado por Contaduría y por Costos/Precios) en vez de uno nuevo — es el patrón que ya siguieron los dos proyectos hermanos.
...
- **Service Layer:** `https://<host>:50000/b1s/v1`. Se probó contra `192.168.1.240` — **sospechado de producción, no confirmado**. Existe una segunda IP mencionada como test (`10.10.10.240`) nunca validada. **`TO VERIFY` — bloqueante antes de apuntar nada a un ambiente de desarrollo real:** confirmar con Germán cuál IP es cuál.
...
- **Usuario de servicio:** `PontynSL`, dedicado (no personal). `KNOWN` que existe; `TO VERIFY` el detalle exacto de permisos otorgados (¿alcanza con lectura + los writes específicos que se necesiten, o tiene de más?).
...
3. **`NUEVO 16/09/2026` Conseguir el texto real de `SP_TransactionNotification`** (o el `.sql` de `hana_reader.py` de `sap_data_access`) para validar que la réplica en `PedidosParaAutorizar` es fiel — no asumirlo sin comparar.
4. **`NUEVO 16/09/2026` Conectividad y credenciales de HANA directo** — distinta de todo lo probado hasta ahora (Service Layer, puerto 50000). Confirmar con Germán usuario de HANA y red/VNet para esto específicamente.
...
6. Confirmar cuál IP (`192.168.1.240` vs `10.10.10.240`) es el ambiente de desarrollo/test vigente — ver Sección 4.1.
