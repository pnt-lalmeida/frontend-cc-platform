...
17. **`GET /api/bandeja/candidatos` sigue devolviendo las filas crudas de HANA** (columnas en español, con tildes, tal como las nombra el `.sql`), sin pasar por ninguna capa de normalización — inconsistente con el resto de la plataforma (Cliente 360 normaliza todo a snake_case y nunca expone nombres de columna de SAP). Ya no depende del punto 16 (resuelto) — queda como mejora pendiente para cuando se construya esa capa.
