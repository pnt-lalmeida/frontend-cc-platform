...
30. **`DENTRO DE ALCANCE, confirmado por Líber 18/09/2026`** — agregar una consulta de "estado de cuenta" de un cliente a Cliente 360, más allá de la lista de facturas abiertas que ya existe. El reporte real (Crystal Report, formato confirmado con captura de Claudia) tiene columnas: Fecha, Vencimiento, Tipo (Factura/N-C), Nro Doc, Moneda, Importe, Pendiente, Saldo corrido, Vendedor. **La pantalla de Cliente 360 ya construida (`GET /api/clientes/{card_code}/facturas`) hoy solo devuelve facturas abiertas** (`Invoices` con `DocumentStatus=bost_Open`) **sin Notas de Crédito, sin `Moneda` por fila, sin `Pendiente` (importe parcialmente pagado), sin `Saldo corrido` (acumulado), sin `Vendedor`.** No es un bug — fue el alcance mínimo elegido a propósito — pero el reporte real usa una consulta SQL propia (no necesariamente `list_open_invoices` de Service Layer) que Líber va a aportar; **no implementar nada de esto hasta tener esa SQL real**, mismo criterio que con `PedidosParaAutorizar`.
