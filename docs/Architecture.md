...
- **`PENDIENTE` — Resource Group, Key Vault, Function App:** Líber decidió no crearlos todavía — se crean cuando se llegue al spike de autenticación / primer deploy real. Queda como plan documentado arriba, con el App Service Plan corregido a `ASP-PontynHttpFunctionApp` (no `asp-pontyn-functions-prod`), pendiente de re-confirmación (ver tabla arriba).
...
8. **`RESUELTO 17/09/2026`** — `U_ClasifClienteCC` es línea de crédito: A = mejor cliente, B = intermedio, C = malo (confirmado por Líber). **`A CONFIRMAR` residual:** el caso real `C3-12788` (Sección 4.3) tiene `CreditLimit=0` y deuda enorme pero `U_ClasifClienteCC="A"` — contradice esta lectura para ese cliente puntual. No usar el campo como única señal de riesgo sin entender esa discrepancia (¿campo no actualizado automáticamente? ¿caso excepcional?).
...
20. **`RESUELTO`** — `GET /api/clientes?q=<texto>` (nuevo): busca por nombre o código contra `BusinessPartners`. Implementación real usa `substringof('...',CardName) or substringof('...',CardCode)` (sintaxis OData v2, coherente con el resto del cliente de Service Layer) — **`TO VERIFY`: nunca probada contra el Service Layer real**, solo contra fixtures.
